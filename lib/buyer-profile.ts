...
// ============================================
// USER PROFILE (profiles table) OPERATIONS
// ============================================

/**
 * Update user profile in the main profiles table
 * This updates username and phone which are shared across the platform
 */
...
export async function updateBuyerAvatar(userId: string, avatarUrl: string) {
  ...
  const { data, error } = await supabase
    .from("buyer_profiles")
    .upsert({ user_id: userId, avatar: avatarUrl }, { onConflict: "user_id" })
    .select()
    .single();
